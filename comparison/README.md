...
- 性价比从当前表中选出“相同或更低综合价格下不存在更高 AA 指数模型”的 5 个 Pareto 前沿模型，拟合得到 `预期 AA 指数 = 42.7721 + 2.8510 × ln(综合价格)`（R² = 0.988），最后计算“实际 AA 指数 / 预期 AA 指数 × 98.2687%”。该系数按当前模型集合中的最高原始性价比归一化，使最高分恰为 100%；其余分数均落在 0–100% 区间。该拟合是当前模型集合内的相对比较，模型或价格变化后需要重新筛选 Pareto 前沿并整体重算。
- Pareto模型的性价比已经被加粗。
...
| Meta Muse Spark 1.3 | -- | -- | -- | 50.5% | 47.5% | 26.0% | 48 |
...
- 数据核对日期：2026-09-19；AA 指数版本：v4.3。
