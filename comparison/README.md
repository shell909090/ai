...
- 性价比从当前表中选出“相同或更低综合价格下不存在更高 AA 指数模型”的 7 个 Pareto 前沿模型，拟合 `预期 AA 指数 = 41.2349 + 3.0547 × ln(综合价格)`（R² = 0.879），最后计算“实际 AA 指数 / 预期 AA 指数 × 95.7207%”。该系数按当前模型集合中的最高原始性价比归一化，使最高分恰为 100%；其余分数均落在 0–100% 区间。无 AA 指数的模型（如 DeepSeek V4.1 Flash）不参与前沿筛选与拟合，性价比从缺。该拟合是当前模型集合内的相对比较，模型或价格变化后需要重新筛选 Pareto 前沿并整体重算。
- Pareto模型的性价比已经被加粗。
