...
| `indexer.py` | 增量扫描：规则匹配筛选 → 文件一致性判断 → 文本抽取 → 语义路由 → raw chunk 或 summary → embed → 写 DB |
...
| `index_route` | `utf8` | 本次索引采用的语义路径：`raw` 或 `summary` |
| `text_entropy` | `float64` | 提取文本的字符级 Shannon entropy |
| `median_paragraph_length` | `int32` | 以空行切段后的典型段落长度中位数 |
...
| `chunk_index` | `int32` | chunk 在文件内的顺序（0-based）；summary 路径固定为 `0` |
| `start` | `int32` | raw 路径为原文起始偏移；summary 路径固定为 `0` |
| `end` | `int32` | raw 路径为原文结束偏移；summary 路径为 `len(summary)` |
| `content_kind` | `utf8` | `raw` 或 `summary`，用于区分检索片段来源 |
| `content` | `utf8` | raw chunk 文本或生成的 summary 文本（用于 regex 过滤和 snippet） |
...
索引单元继续以“原始文件”作为最终返回对象，但 embedding 前新增一个文件级路由步骤：

1. 先用 `all2txt` 提取整份文本。
2. 对整份文本计算：
   - `text_entropy`：字符级 Shannon entropy；
...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `summary_model` | `str` | `"qwen3.5:4b"` | 生成 summary 的聊天/生成模型名，复用现有 OpenAI 兼容接口 |
| `rag_entropy_min` | `float` | `4.5` | 低于该阈值时视为高重复、表格化或模式化文本 |
| `rag_entropy_max` | `float` | `8.8` | 高于该阈值时视为乱码、噪声或非自然语言文本 |
| `rag_min_paragraph_length` | `int` | `80` | 典型段落长度中位数低于该值时，视为碎片化材料 |

### 判定顺序

第一版判定顺序固定如下：

1. 若 `text_entropy < rag_entropy_min`，走 `summary`。
2. 若 `text_entropy > rag_entropy_max`，走 `summary`。
3. 若 `median_paragraph_length < rag_min_paragraph_length`，走 `summary`。
4. 其余情况走 `raw`。

这一定义刻意保持单向、可解释：

- 熵阈值负责排除明显不自然的文本；
- 段落长度负责排除“语义过碎、缺少展开”的文本；
- 只有同时满足“熵正常 + 典型段落足够长”的材料，才允许直接 chunk + embedding。

### summary 生成接口

`Indexer` 新增一个内部摘要步骤，接口定义如下：

```python
def _summarize_text(self, text: str) -> str:
    """Generate a topic summary for indexing without inventing facts."""
```

约束：

1. 输入为整份提取文本，不再额外切 chunk。
...
- `raw` 路径：保持现有行为，`Chunker` 产出多个 raw chunks，`content_kind='raw'`。
- `summary` 路径：不再对原文做 chunk 索引，仅写入一条 summary chunk：
  - `chunk_index = 0`
  - `start = 0`
  - `end = len(summary)`
  - `content_kind = 'summary'`

这样系统仍然只返回原始文件路径，但索引对象从“原文 chunk”切换成“主题 summary”。
...
class Config:
    ...
    top_k: int
    embedding_model: str
    summary_model: str                      # default "qwen3.5:4b"
    ...
    rag_min_paragraph_length: int = 80
...
class Chunker:
    def __init__(self, chunk_size: int = 2048, overlap: int = 200) -> None: ...
