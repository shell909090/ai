...
def test_load_config_ignores_legacy_extractor_field(tmp_path: Path) -> None:
    cfg_file = tmp_path / "config.yaml"
    cfg_file.write_text(
        "dirs:\n"
        "  - path: /photos\n"
    ...
    config = load_config(cfg_file)
    d = config.dirs[0]
    assert d.extractor_config["backends"]["image/jpeg"] == ["openai_vision"]
    assert d.extractor_config["extractors"]["openai_vision"]["model"] == "gpt-4o"
