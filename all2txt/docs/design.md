...
detect(path)
  ...
  │    若 file 不可用或返回 application/octet-stream
  │    → WARNING，fallback 到 mimetypes.guess_type()
  │         若 mimetypes 返回 None
  │         → WARNING，fallback 到内置 _EXT_MAP
  └─ 最终 MIME 若在通用类型列表中，查 Config.extensions 按扩展名覆盖
  │
  ▼
...
**核心原则**：
- 类型识别采用三级探测：`file --mime-type` → `mimetypes` → 内置扩展名映射表 `_EXT_MAP`；`file` 不可用或返回 `application/octet-stream` 时依次 fallback，每级 fallback 均输出 WARNING。
...
class Registry:
    ...
    def detect(self, path: Path) -> str:
        """三级 MIME 探测：file → mimetypes → _EXT_MAP；最终按 Config.extensions 覆盖。"""
...
| InfoExtractor | `info` | 5 | text/x-info, application/x-info | `info` (GNU) |
