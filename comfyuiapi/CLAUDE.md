...
* 放大使用超分过程。目前有两个可选，upscale或USDU。
* upscale过程速度快，效果好，但是只能放大两倍。因此最大像素数只有4\*1.5\*1024\*1024=6Mpx。
* USDU运算速度慢，且可能出现扭曲。但是最大可以支持24Mpx的图片。
* 因此需要智能选择放大过程。我们约定目标width/原图width和目标height/原图height中的最大值, 为放大倍率factor。如果factor<=2，那么使用upscale过程。如果factor>2，使用usdu过程。放大后的文件，称为放大图。
...
4. 第二次计算所有任务，目的是使用upscale放大图片。这次跳过所有“没有超过临界尺寸的”，因为这部分已经生成过了。跳过所有factor>2的，因为这部分要用USDU。
5. 读取原图，生成upscale放大图。
6. 第三次计算所有任务，目的是使用USDU放大图片。这次跳过所有“没有超过临界尺寸的”和factor<=2的，因为这部分已经生成过了。
7. 读取原图，生成USDU放大图。
8. 第四次计算所有任务，目的是把放大图缩小裁切成目标分辨率。这次跳过所有“没有超过临界尺寸的”的，因为这部分已经生成过了。
9. 根据任务的factor，确定目标分辨率需要取用upscale放大图还是USDU放大图。确定后的图统称放大图。放大图尺寸一定大于目标分辨率，但是长宽比可能发生变形（因为分辨率桶）。
10. 使用PIL的ImageOps.fit来适应，具体算法是。保持放大图长宽比固定，将其缩放至“能够覆盖目标尺寸”的最小尺寸。此时一边刚好和目标对齐，而另一边会超出目标。将超出部分从两边裁切(crop)掉即可。
11. 如果目标图尺寸甚至超过了最大放大图尺寸，例如目标尺寸大于24Mpx。那么强行放大。
