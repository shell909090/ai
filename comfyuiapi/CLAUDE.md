...
buckets.png是为此生成的解释图片。它解释了w\*h=1.5Mpx这条曲线上，哪一段归属于那个点。其中1728x960过于接近，主要是16:9的屏幕具有大量占有率导致的。
...
8. 裁剪后直接保存为jpg格式（不先存png再转换）。裁剪成本低，可随时重新生成。文件约定为`{文件编号}_{批次编号}_{device_id}.jpg`。
