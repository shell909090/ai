...
1. 第二轮计算所有任务，目的是使用upscale放大图片。这次跳过所有“没有超过临界尺寸的”，因为这部分已经生成过了。跳过所有factor>2的，因为这部分要用USDU。
2. 读取原图，生成upscale放大图。
3. 第三次计算所有任务，目的是使用USDU放大图片。这次跳过所有“没有超过临界尺寸的”和factor<=2的，因为这部分已经生成过了。
4. 读取原图，生成USDU放大图。
5. 第四次计算所有任务，目的是把放大图缩小裁切成目标分辨率。这次跳过所有“没有超过临界尺寸的”的，因为这部分已经生成过了。
6. 根据任务的factor，确定目标分辨率需要取用upscale放大图还是USDU放大图。确定后的图统称放大图。放大图尺寸一定大于目标分辨率，但是长宽比可能发生变形（因为分辨率桶）。
