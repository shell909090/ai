...
- 对话历史以 flat list 顺序存储；旧 turn 由 compressor 转为 summary 字符串。
...
`Node` 是 `types.py` 中的 Protocol（无具体基类），三个具体节点类各自独立 `@dataclass(slots=True)`，通过结构（structural typing）满足 Protocol：
...
2. 节点整体逻辑 append-only。`UserPromptNode` 和 `AssistantNode` 追加后内容不再变化。`ToolResultNode` 例外：追加时 `results` 为空，tools 并发执行期间原地回填，全部结果到位后才稳定（此时 node 尚未对外暴露，无并发读者）。
...
4. 每个 Node 类实现 `to_anthropic() -> list[dict[str, Any]]` 和 `to_openai() -> list[dict[str, Any]]`，把节点转换为对应 provider 的消息列表（空列表表示该节点不产生消息）。
...
1. **token 阈值（首选）**：本 turn 最后一次 backend 调用的 `usage.input_tokens + usage.output_tokens` 与该 backend 的 `context_window` 之比超过 `compress_threshold`。由 agent 配置项 `agent.compress_threshold` 注入，默认 `0.75`，取值 (0, 1]。
2. **字符兜底**：本 turn 无有效 usage，累加 `session.messages` 各节点序列化字节数（UTF-8）除以 3 估算 token，比较 `compress_threshold`。
...
由 `SessionSearchProvider`（`little_agent/tools/session_search.py`）暴露，仅在顶层配置 `session_store:` 存在（或 web frontend 自动启用）时注册。配合「`session.summaries` 作历史摘要、`search_session` 作 JSONL 正文检索」分工：compressor 把旧 turn 压缩为摘要字符串存入 `session.summaries` 后，AI 通过本工具按需从 JSONL 文件检索原始内容。
...
4. 启动时若用户未配置顶层 `session_store:`，启动脚本以默认参数（`sessions_dir = frontend.sessions_dir` 或 `~/.local/state/little_agent/sessions/`）自动实例化 `SessionJSONLStore`，注入 `agent.hooks`，并注册对应 `SessionSearchProvider` 到 `ToolRegistry`，日志 INFO 提示「session_store auto-enabled for web frontend」；用户已配置则不覆盖。`session/resume` 通过 store 的 `load_history()` 读取节点链。
...
**静态资源**：前端 TypeScript 源码位于顶层 `frontend/`，构建产物输出到 `frontend/dist/`，通过 `make package-static` 拷贝到 `little_agent/_static/`（package data）。运行时 `server.py` 优先从 `little_agent/_static/` 提供静态文件；若未构建则回落到 `little_agent/frontends/static/`（开发兼容路径）。当前无配置项覆盖，路径硬编码。
...
tools:
  providers:
    ...
    little_agent.tools.http.HttpToolProvider: {}     # 需显式列出
    little_agent.tools.file.EditFileToolProvider: {} # 需显式列出
...
session_store:                           # optional；存在时启动脚本同时注入 hooks（SessionJSONLStore）和 tools（SessionSearchProvider）
  sessions_dir: ~/.local/state/little_agent/sessions/
