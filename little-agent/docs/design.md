...
客户端消息：`session/list`、`session/new`、`session/resume`、`session/fork`、`session/delete`、`session/prompt`、`session/cancel`。`session/resume` 服务端先回 resume 确认，再发一条 `session/history`，节点数据来自 `SessionJSONLStore` plugin 写入的 JSONL 文件。
