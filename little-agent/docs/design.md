...
4. 当前是否存在活跃 turn
5. 当前 turn 是否已请求取消
6. pending prompt 队列（容量默认 3，满则抛 `SessionBusyError`）
...
4. tool 执行抛出的异常由编排层的 `except Exception` 统一捕获，写入对应 `call_id` 的结果中（`status = "failed"`，`content` 为异常信息）。**异常绝不冒出 prompt turn**，模型拿到失败反馈后自主决定下一步。
...
1. 若当前 session 已有活跃 turn：进入 pending 队列；队列满则抛 `SessionBusyError`。否则立刻占据，标记"有活跃 turn"。
...
6. `Session` 内部维护 pending prompt 队列，容量默认 3。活跃 turn 存在时，新的 `prompt()` 调用进入队列等待；队列满则抛 `SessionBusyError`。
...
tools:
  task_tool: true                        # optional, default true; set false to disable create_task tool
  providers:
    - type: python
...
10. 有活跃 turn 时再次 `prompt()`，进入 pending 队列；队列满抛 `SessionBusyError`。
